...
dockerCommandPart = f'docker run -it --rm -v "{sourceDirectory}":/bids -v "{outputDirectoryQSI}":/output pennlinc/qsiprep:latest'
...
for subjID in os.listdir(sourceDirectory):
    ...
    fullCommandQSI = f'{dockerCommandPart} {qsiCommandPart}'
